...
function prepareHashes(...hashArr: string[]) {
...
function fetchFiles(filterDirectory: boolean = false, directory: string = '.') {
  // Let's get all the paths and remove the
  // relative path prefix `./`
  const results = runProcess(`find ${directory} -type ${filterDirectory ? 'd' : 'f'}`);
...
export function getChecksums(directory?: string) {
  if (!directory) {
    ...
    const directories = fetchFiles(true);
    ...
    const files = fetchFiles(false, directory).join(' ').trim();
    if (!files || files.length === 0) return;

    const sums = runProcess(`find ${files} -exec sha256sum {} +`);
    return prepareHashes(sums);
  }
